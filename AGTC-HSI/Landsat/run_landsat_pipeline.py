"""
Complete pipeline script for Landsat hyperspectral image restoration
...
def prepare_data():
    ...
    data_prep_script = Path(__file__).parent / "Data-Preparation" / "train_pairs_creator_Landsat.py"
...
def train_model(epochs=100, batch_size=1, checkpoint_freq=2):
    ...
    train_data_path = Path(__file__).parent / "Data-Preparation" / "Train_Pairs_Landsat"
    ...
    print(f"  - Input dimensions: 8 (Landsat bands)")
...
def test_model(ground_truth_path=None):
    ...
    weight_path = Path(__file__).parent / "Weight" / "AGTC-Landsat.pth"
    test_data = Path(__file__).parent / "Landsat_test.mat"
...
def main():
    parser = argparse.ArgumentParser(
        description="Complete pipeline for Landsat hyperspectral image restoration"
    )
    parser.add_argument(
        "--mode",
    ...
    print("\n" + "="*70)
    print("LANDSAT HYPERSPECTRAL IMAGE RESTORATION PIPELINE")
    print("="*70)
