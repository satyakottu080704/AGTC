...
def train(opt):
        ...
        if (epoch % opt.checkpoint_freq) == 0:
            torch.save(
                {'epoch': epoch, 'model_state_dict': model.state_dict(),
                 ...
                 'loss': epoch_avg_loss},
                os.path.join(opt.save_path, f'AGTC-PaviaU_epoch_{epoch}.pth')
            )
    ...
    torch.save(
        {'epoch': 100, 'model_state_dict': model.state_dict(),
         'optimizer_state_dict': optimizer.state_dict()},
        os.path.join(opt.save_path, 'AGTC-Pavia.pth')
    )
    print(f'Final model saved to {opt.save_path}/AGTC-Pavia.pth')
